...
def WriteAlerts(alertList, alertFile):
    ...
    with open(alert_path,"w") as file:
        json.dump(alertList, file,indent=4,separators=(',',': '))


def UpdateAlert(alert):
    alertFile="data.json"
    alertList=GetAlerts(alertFile)
...
def nvanalytics_src_pad_buffer_probe(pad, info, u_data):
    global person_count_1, person_count_2, person_count_3
    frame_number = 0
    num_rects = 0
    ...
    while l_frame:
                        ...
                        if user_meta_data.roiStatus == ['RF-0']:
                            try:
                                if (obj_meta.class_id == 0 and obj_meta.object_id not in roi_tracker_1):
                                    ...
                                    detected_date = datetime.datetime.now().strftime("%m/%d/%Y")
        ...
        py_nvosd_text_params.display_text = "Person Count={}".format(user_meta_data.objInROIcnt)
...
def main(args):
    ...
    global perf_data
    global roi_tracker_1, roi_tracker_2, roi_tracker_3
    roi_tracker_1 = []
    roi_tracker_2 = []
    roi_tracker_3 = []
